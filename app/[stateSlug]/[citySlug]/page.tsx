...
export async function generateMetadata({
  ...
  const title = `Neurologists in ${cityName}, ${stateName} | Neurologist Directories`;
  const description = `Find trusted neurology care in ${cityName}, ${stateName}—browse ${count.toLocaleString()} verified practices with contact details, maps, and Google ratings so you can choose with confidence.`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${cityName}, ${stateName} neurologist directory preview`,
        },
      ],
    },
  };
...
export default async function CityPage({ params }: CityPageProps) {
  ...
  const webpageSchema = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: `Neurologists in ${cityName}, ${stateName}`,
    ...
    about: [
      {
        "@type": "Thing",
        name: `${cityName} neurologists`,
      },
      {
        "@type": "Thing",
        name: `${stateName} neurology services`,
      },
      {
        "@type": "Thing",
    ...
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <header className="space-y-4">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-teal">
          Neurologists by city
        </p>
        <h1 className="text-3xl font-semibold text-navy">
          Neurologists in {cityName}, {stateName}
        </h1>
        <p className="max-w-2xl text-sm text-slate-600">
          {cityName} has {facilities.length.toLocaleString()} verified neurology
        ...
        <p className="max-w-2xl text-sm text-slate-600">
          ...
          and find the right neurologist for you and your family in{" "}
          {stateName}.
        </p>
      </header>
