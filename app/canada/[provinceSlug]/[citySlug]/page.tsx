...
export async function generateMetadata({
  ...
  const title = `Neurologists in ${cityName}, ${provinceName}, Canada | Neurologist Directories`;
  const description = `Find ${count.toLocaleString()} neurologists and neurology practices in ${cityName}, ${provinceName}. Compare services and practice details. Verified listings with ratings and reviews.`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${cityName}, ${provinceName} neurologist directory preview`,
        },
      ],
    },
  };
...
export default async function CanadaCityPage({ params }: CanadaCityPageProps) {
  ...
  const webpageSchema = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: `Neurologists in ${cityName}, ${provinceName}, Canada`,
    ...
    about: [
      { "@type": "Thing", name: `${cityName} neurologists` },
      { "@type": "Thing", name: `${provinceName} neurology services` },
    ...
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <header className="space-y-4">
        ...
        <h1 className="text-3xl font-semibold text-navy">
          Neurologists in {cityName}, {provinceName}
        </h1>
        <p className="max-w-2xl text-sm text-slate-600">
          {cityName} has {facilities.length.toLocaleString()} verified
          neurology practices including {careTypesText}. Browse all options below,
