 ...
 * (e.g. city page intros). Omits entries that do not look neurology-related.
 */

const EXACT_PHRASE: Record<string, string> = {
  neurologist: "neurologists",
  "pediatric neurologist": "pediatric neurologists",
  neurophysiologist: "neurophysiologists",
  neurosurgeon: "neurosurgeons",
  "child neurologist": "child neurologists",
  neurology: "neurology services",
  "neurology physician": "neurology physicians",
  "clinical neurophysiologist": "clinical neurophysiologists",
};

const NEUROLOGY_LIKE =
  /neurologist|neurophysiologist|neurosurgeon|neurology/i;
...
function humanizeFallback(raw: string): string {
  ...
  if (s.endsWith("ist") && !/neurologist$/.test(s)) {
    return `${s}s`;
  }
  if (!s.endsWith("s")) {
    return `${s}s`;
...
function phraseForLabel(raw: string): string | null {
  ...
  if (!key) return null;
  if (NON_NEUROLOGY.test(key)) return null;
  if (EXACT_PHRASE[key]) return EXACT_PHRASE[key];
  if (!NEUROLOGY_LIKE.test(raw)) return null;
...
export function formatCareTypesClause(
  ...
  if (phrases.length === 0) {
    return "including neurologists, pediatric neurologists, neurophysiologists, and neurosurgeons";
  }
